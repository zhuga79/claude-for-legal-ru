#!/usr/bin/env python3
"""НЕ ИСПОЛЬЗОВАТЬ ДЛЯ РЕШЕНИЯ О ПАУЗЕ. Скрипт показал 42%, когда реальный расход был 100%.

Почему ошибся (2026-08-08): скрипт считает СКОЛЬЗЯЩЕЕ окно в 5 часов назад от текущего момента,
а индикатор в Claude Code считает ФИКСИРОВАННЫЙ блок сессии до момента сброса. Это разные
величины: из скользящего окна старые запросы выпадают, из блока — нет. Плюс из суммы исключено
чтение кэша (в окне — свыше 100 млн токенов против 8 млн всего остального), а оно, судя по
расхождению, в лимит попадает.

Достоверный источник — строка состояния Claude Code: [5h:NN% 7d:NN%] и экран /usage.
Скрипт оставлен только как разбор расхода по сессиям и подагентам.
